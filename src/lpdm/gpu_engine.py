...
VALIDATE_ENGINE_INPUTS = os.environ.get("GLIDE_VALIDATE_ENGINE", "") not in (
    "",
    "0",
    "false",
    "False",
    ...
    def __init__(
        ...
        *,
        ...
        if compile_hot_paths is None:
            compile_hot_paths = os.environ.get("GLIDE_COMPILE", "") not in (
                "",
                "0",
                "false",
                "False",
    ...
    def normalize_particle_coordinates(
        ...
        *,
        ...
        if level_arr is not None:
            ...
            if n_levels < 2:
                raise ValueError(
                    "level_agl_m must have at least 2 entries for vertical interpolation"
                )
            if not ascending:
            ...
            idx_upper = torch.searchsorted(level_arr_sorted, alt.contiguous()).clamp(
                min=1, max=n_levels - 1
            )
