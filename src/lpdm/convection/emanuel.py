...
- We use the bbox-mean met column (`metadata.level`) for the parcel lift,
  ...
  uniformly across the bbox — a strong approximation for large domains.
  Advection's vertical interpolation makes the same approximation; both are
  open follow-ups in STATUS.md.
"""
...
def compute_lcl_lnb_cape(
    ...
    t_lcl = lcl_temperature_bolton(
        t_sfc.unsqueeze(0), q_sfc.unsqueeze(0), p_sfc.unsqueeze(0)
    ).squeeze(0)
    p_lcl = lcl_pressure_poisson(
        t_sfc.unsqueeze(0), t_lcl.unsqueeze(0), p_sfc.unsqueeze(0)
    ).squeeze(0)
...
class EmanuelReducedConvection(ConvectionScheme):
    ...
    def maybe_convect(
        ...
        *,
        ...
        pressure_pa = (
            torch.as_tensor(
                np.array(met_window.metadata.pressure_level_hpa),
                device=device,
                dtype=dtype,
            )
            * 100.0
        ...
        if self._last_log_t_alpha != t_alpha:
            LOGGER.info(
                ...
                t_alpha,
                float(cape),
                float(height_col[i_lcl]),
                float(height_col[i_lnb]),
