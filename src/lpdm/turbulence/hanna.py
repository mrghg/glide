...
def _in_bl_unstable(
    ...
    sigma_w_sq = 1.2 * w_star.pow(2) * (1.0 - 0.9 * z_over_h).clamp(min=0.0) * z_over_h.pow(
        2.0 / 3.0
...
def in_bl_sigma_TL(
    ...
    def _select(
        stable_v: torch.Tensor, neutral_v: torch.Tensor, unstable_v: torch.Tensor
    ) -> torch.Tensor:
...
def free_trop_diffusivity(
    ...
    * `f(Ri)` — stability function: `(1 - Ri/Ri_c)²` for `0 ≤ Ri < Ri_c`,
    ...
    mixing_length = (KARMAN * z) / (1.0 + KARMAN * z / FT_MIXING_LENGTH_ASYMPTOTE_M)
    ...
    k_z = mixing_length.pow(2) * shear_mag * f_ri
...
class HannaScheme(TurbulenceScheme):
    ...
    def step(
        ...
        blh = self._interp_surface_field(
            met_window, "blh", t_alpha, lon_deg, lat_deg, device, dtype
        ).clamp(min=BLH_MIN_M)
        sp = self._interp_surface_field(met_window, "sp", t_alpha, lon_deg, lat_deg, device, dtype)
        ustar = self._interp_surface_field(
            met_window, "ustar", t_alpha, lon_deg, lat_deg, device, dtype
        ).clamp(min=USTAR_MIN_M_S)
        shf = self._interp_surface_field(
            met_window, "shf", t_alpha, lon_deg, lat_deg, device, dtype
        )
        t_surface = self._interp_t_at_lowest_level(
            met_window, t_alpha, lon_deg, lat_deg, device, dtype
        )
    ...
    def _gather_static_inputs(self, met_window, t_alpha, device, dtype):
        ...
        if (
            alpha_buf is None
            or not self._device_matches(alpha_buf.device, device)
            or alpha_buf.dtype != dtype
        ):
    ...
    def _advect_rk2(
        ...
        def wind(xyz: torch.Tensor) -> torch.Tensor:
            xyz_norm = engine.normalize_particle_coordinates(
                xyz, grid_bounds, level_arr=level_arr, ascending=ascending
            )
            grid = xyz_norm.view(1, 1, 1, -1, 3)
            v_start = (
                torch.nn.functional.grid_sample(m_start, grid, align_corners=True).view(3, -1).t()
            )
    ...
    def _step_core(
        ...
        *,
        ...
        blh = self._bilinear_at(blh_field, lon0, lon1, lat0, lat1, lon_deg, lat_deg).clamp(
            min=BLH_MIN_M
        )
        sp = self._bilinear_at(sp_field, lon0, lon1, lat0, lat1, lon_deg, lat_deg)
        ustar = self._bilinear_at(ustar_field, lon0, lon1, lat0, lat1, lon_deg, lat_deg).clamp(
            min=USTAR_MIN_M_S
        )
        ...
        if self.meander_enabled:
            ...
            moved = engine.apply_horizontal_turbulence(
                moved, u_meander_new, v_meander_new, dt_seconds=dt_seconds, backward=True
            )
    ...
    def _bilinear_at(field_2d, lon0, lon1, lat0, lat1, particle_lon, particle_lat):
        ...
        grid = (
            torch.stack([lon_norm, lat_norm], dim=-1)
            .view(1, 1, -1, 2)
            .to(device=field_2d.device, dtype=field_2d.dtype)
        )
    ...
    def _substep_counts(
        ...
        k_required = (
            torch.ceil(dt_seconds / (c * T_Lw.clamp(min=1e-3))).long().clamp(min=1, max=k_cap)
        )
    ...
    def _integrate_vertical_substeps_static(
        ...
        **Not bit-identical** to the dynamic variant when ``k_i`` varies across
        ...
        for i in range(n_iter):
            ...
            xyz = engine.apply_horizontal_turbulence(
                xyz, u_new, v_new, dt_seconds=sub_dt_i, backward=True
            )
        ...
        *,
        ...
        for i in range(max_k):
            ...
            u_new = engine.update_langevin_velocity(
                u_p[mask],
                t_lagrangian=T_Lu_m,
                sigma_w2=sigma_u_m.pow(2),
                dt_seconds=sub_dt_m,
            )
            v_new = engine.update_langevin_velocity(
                v_p[mask],
                t_lagrangian=T_Lv_m,
                sigma_w2=sigma_v_m.pow(2),
                dt_seconds=sub_dt_m,
            )
            w_new = engine.update_langevin_velocity(
                w_p[mask],
                t_lagrangian=T_Lw_m,
                sigma_w2=sigma_w_m.pow(2),
                dt_seconds=sub_dt_m,
                drift=drift_w_m,
            )
    ...
    def _to_static_buffer(
        self, name: str, src: torch.Tensor, *, copy_src: bool = True
    ) -> torch.Tensor:
    ...
    def _windowed_std(field: torch.Tensor, radius: int) -> torch.Tensor:
        ...
        mean = torch.nn.functional.avg_pool2d(
            x, k, stride=1, padding=radius, count_include_pad=False
        )
        mean_sq = torch.nn.functional.avg_pool2d(
            x * x, k, stride=1, padding=radius, count_include_pad=False
        )
    ...
    def _interp_surface_field(
        ...
        return cls._interp_2d_bilinear(
            field_2d,
            met_window.metadata.lon,
            met_window.metadata.lat,
            lon,
            lat,
        )
