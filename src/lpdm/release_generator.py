...
class PointRelease(Release):
    ...
    def generate(self) -> torch.Tensor:
        ...
        weight = torch.full(
            (self.n_particles,), 1.0 / self.n_particles, dtype=self.dtype, device=self.device
        )
...
class VolumeRelease(Release):
    ...
    def generate(self) -> torch.Tensor:
        lon = torch.empty(self.n_particles, dtype=self.dtype, device=self.device).uniform_(
            self.lon_min, self.lon_max
        )
        lat = torch.empty(self.n_particles, dtype=self.dtype, device=self.device).uniform_(
            self.lat_min, self.lat_max
        )
        alt = torch.empty(self.n_particles, dtype=self.dtype, device=self.device).uniform_(
    ...
    * Each particle's absolute release time is ``release.release_time + within``,
    ...
    for rel in batch.releases:
        ...
        if rel.seed is not None:
            ...
            within = (
                torch.empty(n, device="cpu", dtype=torch.float32)
                .uniform_(0.0, duration_s, generator=rng)
                .to(device=device_t)
            )
        else:
            within = torch.empty(n, device=device_t, dtype=torch.float32).uniform_(0.0, duration_s)
