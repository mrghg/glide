...
- **TODO #2 — `torch.compile` readiness (deferred).** The substep loop launches many tiny elementwise kernels per step; on GPU the launch overhead can dominate. The transferable optimisation (vs the other project's bf16/TF32/fused-EMA, which are ML-training-specific and *not* applicable here) is kernel **fusion** via `torch.compile` (Inductor/Triton backend — needs the `cudatoolkit/24.11_12.6` module on Isambard AI for nvcc/ptxas). Scaffold this behind a `--compile` / config flag with graceful fallback, and pair it with removing the remaining `active_count` control-flow sync so the hot loop has no host syncs. Keep float32 (bf16 would wreck position accumulation and the finite-difference WMC drift terms).
