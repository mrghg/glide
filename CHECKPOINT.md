...
- **Streaming per-batch Zarr writes** stay on the M5 follow-up list (already flagged in the 2026-05-27 stage-2-to-5 entry). With streaming the gridder would allocate only one batch's slice at a time, lifting the constraint for runs that genuinely need higher `n_time_bins`. Not done yet; the guard + the FLEXPART-friendly config default cover the immediate use case.
...
- **Milestone status:** M0 complete. M1 implementation-complete (default still `placeholder_constant_ou` pending external validation). M4 mostly complete — schema, output_grid/met_domain split, and CLI shrink all landed; remaining gaps are `schema_version` field and explicit out-of-domain particle handling (see 2026-05-24 entry). **M5 stages 1–5 complete (2026-05-26 + 2026-05-27)**: multi-release configs now execute end-to-end, producing a single 5D `footprints.zarr` indexed by `release_time`. `configs/example_mhd_january_periodic.yaml` is the 744-footprint reference run.
...
- **M5 follow-ups (optional, can land any time):**
  - Streaming per-batch Zarr writes (today: gridder allocates the full 5D tensor in RAM; at full FLEXPART resolution × 744 releases that's ~23 GB). Worth doing when grid resolution × n_releases pushes past available RAM/VRAM.
